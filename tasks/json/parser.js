...
function parse(jsonFile) {
    const parsedJson = jsonFile.list.entries.map(o => {
            return {docId: `http://doc.epam.com/${(o.entry.name.replace(".html", ""))}`}
        });

    fs.writeFile('./parsed.json', JSON.stringify(parsedJson), err => {
        if (err) {
            throw err;
        }
